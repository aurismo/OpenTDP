...
def validate(package: TDPPackage) -> List[str]:
    ...
    if not package.version.unique_id.strip():
        errors.append("Unique ID (UUID) is required.")
    ...
    # 6c. Approved / Released, Low+: Approver, Approval Conditions and
    #     Evidence References required. Non-critical is exempt — its
    #     approval trail stays informational only (spec section 2/8).
    if _at_least_lifecycle(status, "Approved") and _at_least_criticality(level, "Low"):
        ap = package.quality.approval
        if not ap.approver.strip():
            errors.append(
                "Quality: Approver is required "
                "(Approved/Released, Low criticality+)."
            )
        if not ap.conditions.strip():
            errors.append(
                "Quality: Approval Conditions is required "
                "(Approved/Released, Low criticality+)."
            )
        if not ap.evidence_refs.strip():
            errors.append(
                "Quality: Evidence References is required "
                "(Approved/Released, Low criticality+)."
            )

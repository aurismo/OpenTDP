...
def to_xml_string(package: TDPPackage) -> str:
    # Root element must carry the schema version attribute per spec 1.3.
    root = ET.Element("TDP")
    root.set("version", "1.0")
    # Element order follows spec 1.3's documented top-level structure:
    # Metadata, Version, Quality, Geometry, ManufacturingOverview,
    # ProcessOptions, BuildFiles, PackingAndSafety, OptionalData.
...
def _write_overview(root: ET.Element, ov: ManufacturingOverview) -> None:
    ...
    for key, val in d.items():
        if key == "Standards":
            ...
            for s in val:
                _sub(std_root, "Standard", s, "13040")
        else:
...
def _write_step_element(parent: ET.Element, sd: dict) -> None:
    ...
    for key, val in sd.items():
        if key == "attachments":
            ...
            for v in val:
                _sub(inp, "Input", v, "15070")
        elif key == "outputs":
            out = ET.SubElement(s, "Outputs")
            for v in val:
                _sub(out, "Output", v, "15080")
        elif key == "build_files":
            bf_el = ET.SubElement(s, "BuildFileRefs")
            for v in val:
                _sub(bf_el, "Ref", v, "15090")
        elif key == "duration":
            dur = ET.SubElement(s, "Duration")
            _sub(dur, "Hours", str(val.get("hours", 0)), "15100")
            _sub(dur, "Minutes", str(val.get("minutes", 0)), "15110")

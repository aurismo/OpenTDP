...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("OpenTDP Package Editor")
    ...
    def _open_info(self):
        ...
        dlg = QDialog(self)
        dlg.setWindowTitle("About OpenTDP Package Editor")
        ...
        layout.addWidget(_lbl('OpenTDP Package Editor', size=18, bold=True,
                               align=Qt.AlignmentFlag.AlignCenter))
        layout.addWidget(_lbl('Version v20260909a  —  Alpha Release', size=11, colour='#374151',
                               align=Qt.AlignmentFlag.AlignCenter))
        layout.addWidget(_lbl(
            'This is an Alpha version of both the OpenTDP Package Editor application '
        ...
        layout.addWidget(_lbl('https://opentdp.eu', size=10, colour='#0369a1'))
        layout.addWidget(_hr())
    ...
    def _open_bundled_file(self, filename: str, label: str):
        """Open a file bundled next to main.py in the system default viewer."""
        ...
        candidate = os.path.join(os.path.dirname(base), filename)
        ...
        try:
            ...
            QMessageBox.warning(self, label, f"Could not open file:\n{e}")

    def _open_specification(self):
        """Open the bundled TDP Specification PDF in the system default viewer."""
        self._open_bundled_file("TDP_Specification_2026-09-19.pdf", "Specification")

    def _open_schema(self):
        """Open the bundled TDP.xml XML Schema (XSD) in the system default viewer."""
        self._open_bundled_file("TDP_Schema_2026-09-19.xsd", "Schema")
    ...
    def _new(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self._dirty = False
            self.setWindowTitle("OpenTDP Package Editor — New Package")
    ...
    def _load(self):
        ...
        self._populate(pkg)
        self._dirty = False
        # Pass TDP path to tabs that support file export
        for tab in (self.geometry_tab, self.amdata_tab, self.optional_tab):
            tab._current_tdp_path = path
        self.setWindowTitle(f"OpenTDP Package Editor — {path}")
    ...
    def _do_save(self, path: str):
        ...
        try:
            tdp_io.save_tdp(path, pkg, author=author, history_source_dir=self._extract_dir)
        except tdp_io.TDPSaveError as exc:
            QMessageBox.critical(self, "Save Error", str(exc))
            return

        # Reload from saved file so ChangeHistory reflects what was written,
        # and point _extract_dir at THIS save's extraction so that the next
        # save (even without an explicit reload in between) still finds
        # every History/ entry written so far, not a stale pre-save copy.
        try:
            saved_pkg, new_extract_dir = tdp_io.load_tdp(path)
            self.version_tab.load_data(saved_pkg.version)
            self._cleanup_extract_dir()
            self._extract_dir = new_extract_dir
        except Exception:

...
class ManufacturingTab(QWidget):
    ...
    def _edit_option(self):
        ...
        if dlg.exec():
            ...
            # Keep the existing Step objects exactly as they are (including
            # each Attachment's source_path). Previously this round-tripped
            # steps through to_xml_dict()/from_xml_dict(), which is correct
            # for what gets written to TDP.xml but silently discards
            # runtime-only fields like source_path — after which attachments
            # could never be packed into the .tdp or exported again.
            updated.steps = self._options[idx].steps
    ...
    def _refresh_steps_list(self):
        ...
        if self._current_option_idx is None:
            return
        # Sort by (Sequence Number, Sequence Letter, Step ID) — letter-less
        # steps sort before lettered ones at the same number; ID is the
        # final tie-break when both number and letter are equal. Sorting
        # the underlying list too (not just the display) keeps the saved
        # XML/JSON in the same order as what's shown here.
        sorted_steps = sorted(
            self._options[self._current_option_idx].steps,
            key=lambda s: (s.sequence, s.sequence_letter, s.id)
        )
        self._options[self._current_option_idx].steps = sorted_steps
        for s in sorted_steps:
            label = f"{s.sequence:03d}{s.sequence_letter}"
            item = QListWidgetItem(f"{label}  {s.name}  [{s.id}]")
    ...
    def _add_step(self):
        ...
        dlg = StepEditor(
            ...
            option_id=self._options[self._current_option_idx].id,
            existing_steps=self._options[self._current_option_idx].steps,
        )
        dlg._current_tdp_path = self.mainwindow._current_path
        if dlg.exec():
            step = dlg.get_data()

...
class StepEditor(QDialog):
    def __init__(self, parent=None,
                 ...
                 option_id: str = "",
                 existing_steps: list[ProcessStep] = None):
        ...
        # Other steps already in this Process Option — used only to
        # suggest the next free Sequence Number for a brand-new step.
        self._existing_steps = existing_steps or []
        # Parent Process Option's stable id — used to place this step's
        # attachments under Manufacturing/<option_id>/<step_id>/ inside
        # the .tdp archive (see services.tdp_io.attachment_arc_dir).
    ...
    def _make_tab(self):
        ...
        The form lives in its own inner widget with a trailing stretch
        below it, so any leftover vertical space (the tab area is taller
        than its content needs) collects at the bottom of the tab instead
        of being spread out between the rows themselves.
        """
        w = QWidget()
        outer = QVBoxLayout(w)
    ...
    def _paired_row(self, label1: str, widget1, label2: str, widget2) -> QWidget:
        """Two label+widget pairs side by side, added to a form as one row."""
        container = QWidget()
    ...
    def _suggested_sequence(self) -> int:
        """Next free Sequence Number for a brand-new step in this option."""
        if not self._existing_steps:
            return 1
        return max((s.sequence for s in self._existing_steps), default=0) + 1
    ...
    def _tab_basics(self):
        ...
        if self._existing is None:
            # New step: suggest the next free number in this Process
            # Option so steps don't all default to 0.
            self.sequence.setValue(self._suggested_sequence())
        self.sequence_letter = QComboBox()
        self.sequence_letter.addItem("")  # no letter — default, sequential step
        self.sequence_letter.addItems(list("ABCDEFGHIJKLMNOPQRSTUVWXYZ"))
        self.sequence_letter.setToolTip(
            "Optional: give steps that share the same Sequence Number a "
            "different letter when they can be performed simultaneously."
        )
        self.name = QLineEdit()
        self.description = QTextEdit()
        self.description.setFixedHeight(112)  # 64 * 1.75
        ...
        self.build_file_list.setFixedHeight(140)  # 80 * 1.75
        self.build_file_list.setStyleSheet(
        ...
        # A small gap after Description visually splits the tab into two
        # sections: identity/scope above, execution details below.
        section_gap = QWidget()
        section_gap.setFixedHeight(14)

        form.addRow(self._paired_row("Step ID:", self.step_id, "Name:", self.name))
        form.addRow(self._paired_row("Sequence Number:", self.sequence, "Sequence Letter:", self.sequence_letter))
        form.addRow("Description:", self.description)
        form.addRow(section_gap)
        form.addRow("Manufacturing Method:", self.mfg_method)
        form.addRow("Build Orientation:", self.orientation)
        form.addRow(self._paired_row("Input Geometry:", self.input_combo, "Output Geometry:", self.output_combo))
        form.addRow(self._paired_row("Estimated Duration:", dur_container, "Operators Required:", self.operator_count))
    ...
    def _tab_attachments(self):
        ...
        btn_exp.clicked.connect(self._export_attachment)
        btn_row.addWidget(btn_add)
        btn_row.addWidget(btn_del)
    ...
    def _populate(self, s: ProcessStep):
        ...
        self.sequence_letter.setCurrentText(s.sequence_letter)
    ...
    def _export_file(self, file_name: str, source_path: str,
        ...
        """Save a file either from source_path or from the open .tdp archive."""
        import os, shutil, zipfile
        from PyQt6.QtWidgets import QFileDialog, QMessageBox

        dest, _ = QFileDialog.getSaveFileName(
            self, "Export File", file_name)
        if not dest:
            return

        # 1. Try source_path first (file was added this session)
        if source_path and os.path.isfile(source_path):
            shutil.copy2(source_path, dest)
            QMessageBox.information(self, "Export", f"Saved to:\n{dest}")
            return

        # 2. Try extracting from the open .tdp archive
        ...
        QMessageBox.warning(
            ...
            "The original source file may have been moved or the package\n"
    ...
    def _export_attachment(self):
        ...
        if row < 0:
            ...
            QMessageBox.information(self, 'Export', 'Select an attachment first.')
            return
        item = self.att_list.item(row)
        att = item.data(Qt.ItemDataRole.UserRole) if item else None
        if att is None:
            return
        step_id = self.step_id.text().strip()
        arc_prefix = tdp_io.attachment_arc_dir(self._option_id, step_id)
        self._export_file(att.file_name, att.source_path, arc_prefix)
    ...
    def get_data(self) -> ProcessStep:
        ...
        return ProcessStep(
            ...
            sequence_letter=self.sequence_letter.currentText(),
            name=self.name.text(),

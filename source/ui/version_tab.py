...
from PyQt6.QtWidgets import (
    ...
    QHBoxLayout, QTextEdit, QDateTimeEdit, QLabel, QMessageBox
)
...
class VersionTab(QWidget):
    ...
    def _encryption_group(self):
        ...
        self.encryption_enabled.setEnabled(False)
        ...
        self.encryption_algorithm.setEnabled(False)
        alpha_note = QLabel(
            "⚠  Encryption is not available in this Alpha version."
        )
        alpha_note.setStyleSheet(
            "color:#854d0e; font-size:11px; font-style:italic;"
        )
    ...
    def _history_group(self):
        ...
        self.change_list.itemDoubleClicked.connect(self._open_history_file)
    ...
    def _preview_checksum(self):
        try:
            ...
            sha = tdp_io.compute_package_checksum(package)
    ...
    def _open_history_file(self, item: QListWidgetItem):
        """
        Double-click on a Change History row: extract that entry's
        History/<ChangeID>.txt from the saved .tdp archive to a temp file
        and open it with the OS default text editor.
        """
        entry = item.data(Qt.ItemDataRole.UserRole)
        if not isinstance(entry, ChangeEntry) or not entry.change_id:
            QMessageBox.information(
                self, "History File",
                "This entry has no linked History file to open."
            )
            return

        tdp_path = getattr(self.mainwindow, "_current_path", None)
        if not tdp_path or not os.path.isfile(tdp_path):
            QMessageBox.information(
                self, "History File",
                "Save the package first — the History file only exists\n"
                "inside a saved .tdp archive."
            )
            return

        from services import tdp_io
        arcname = tdp_io.history_arcname(entry.change_id)
        try:
            with zipfile.ZipFile(tdp_path) as zf:
                if arcname not in zf.namelist():
                    QMessageBox.information(
                        self, "History File",
                        f"No History file found for this entry in the saved\n"
                        f"package (expected {arcname}). Save the package\n"
                        f"again to (re)generate it."
                    )
                    return
                data = zf.read(arcname)
        except Exception as exc:
            QMessageBox.warning(self, "History File", f"Could not read the .tdp archive:\n{exc}")
            return

        # Extract to a temp file so the OS's default text editor can open it.
        tmp_path = os.path.join(tempfile.gettempdir(), f"tdp_{entry.change_id}.txt")
        try:
            with open(tmp_path, "wb") as f:
                f.write(data)
        except OSError as exc:
            QMessageBox.warning(self, "History File", f"Could not extract the History file:\n{exc}")
            return

        if not QDesktopServices.openUrl(QUrl.fromLocalFile(tmp_path)):
            QMessageBox.warning(
                self, "History File",
                f"Could not open a default text editor for:\n{tmp_path}"
            )
    ...
    def add_change_entry_to_list(self, entry: ChangeEntry):
        ...
        # Store the actual ChangeEntry object, not just its display text, so
        # get_data() can read every field back exactly (including ones like
        # Checksum that were added after the display format was written)
        # instead of re-parsing a hand-formatted string with a regex.
    ...
    def get_data(self) -> VersionInfo:
        ...
        for i in range(self.change_list.count()):
            item = self.change_list.item(i)
            stored = item.data(Qt.ItemDataRole.UserRole)
            if isinstance(stored, ChangeEntry):
                history.append(stored)
            else:
                # Fallback for any item that somehow has no stored object
                # (e.g. hand-added via the manual entry editor in an older
                # session) — parse it back from the displayed text.
                lines = item.text().splitlines()
                header = lines[0] if len(lines) > 0 else ""
                author_line = lines[1] if len(lines) > 1 else ""
                desc = lines[2] if len(lines) > 2 else ""
                checksum_line = lines[3] if len(lines) > 3 else ""
                import re
                m = re.match(r"\[(.+?)\]\s+(.+)", header)
                ts = m.group(1) if m else ""
                ch_id = m.group(2) if m else ""
                author = author_line.replace("Author: ", "")
                checksum = checksum_line.replace("Checksum: ", "")
                history.append(ChangeEntry(
                    change_id=ch_id, timestamp=ts, author=author,
                    description=desc, checksum=checksum,
                ))
